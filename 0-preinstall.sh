...
# on /dev/sda (nvme ssd)
sgdisk -n 1:0:+1024M /dev/sda  # partition 1 (EFI), 1GB
sgdisk -n 2:0:+16384M /dev/sda # partition 2 (SWAP), 16GB
sgdisk -n 3:0:+51200M /dev/sda # partition 3 (ROOT), 50GB
...
# on /dev/sda (nvme ssd)
sgdisk -t 1:ef00 /dev/sda # making partition 1 type efi
sgdisk -t 2:8200 /dev/sda # making partition 2 type linux swap
sgdisk -t 3:8300 /dev/sda # making partition 3 type linux file system
...
# on /dev/sda (nvme ssd)
mkfs.vfat -F32 -n "UEFISYS" "/dev/sdap1" # formating efi partition with fat.
mkswap -L "SWAP" "/dev/sdap2"            # formating swap partition with linux swap.
mkfs.f2fs -f -l "ROOT" "/dev/sdap3"      # formating root partition with f2fs.
mkfs.f2fs -f -l "HOME" "/dev/sdap4"      # formating home partition with f2fs.
# on /dev/sda (sata ssd)
...
# Mount targets
# mount root
mount "/dev/sdap3" /mnt # mounts root
...
# mount other targets
mount "/dev/sdap1" /mnt/boot # mounts boot
mount "/dev/sdap4" /mnt/home # mounts home
# mount "/dev/sda1" /mnt/data      # mounts data
# mount "/dev/sdb1" /mnt/extra     # mounts extra
swapon "/dev/sdap2"          # turns swap on
...
umount "/dev/sdap1" # umounts boot in order to mount when in arch-chroot for the grub installation to work.
echo "--------------------------------------"
