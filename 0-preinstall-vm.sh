...
# on /dev/sda (nvme ssd)
mkfs.vfat -F32 -n "UEFISYS" "/dev/sda1" # formating efi partition with fat.
mkswap -L "SWAP" "/dev/sda2"            # formating swap partition with linux swap.
mkfs.f2fs -f -l "ROOT" "/dev/sda3"      # formating root partition with f2fs.
...
# Mount targets
# mount root
mount "/dev/sda3" /mnt # mounts root
...
umount "/dev/sda1" # umounts boot in order to mount when in arch-chroot for the grub installation to work.
echo "--------------------------------------"
